...
export default function EditAnimal() {
  ...
  React.useEffect(() => {
    // dispatch(checkAuth());
    dispatch({ type: 'GET_ANIMALS', payload: Number(id) });
  }, []);
  ...
  const editHandler = async (e) => {
    ...
    // console.log(data);
    dispatch({ type: 'UPDATE_ANIMALS', payload: { id: Number(id), data } });
    navigate('/requests');
  };
  ...
  React.useEffect(() => {
    setGender(ani?.gender);
    setAnimaltype(ani?.animal_type);
    setBreed(ani?.breed);
    setColor(ani?.color);
    setPricecategory(ani?.price_category);
    setPeriodcategory(ani?.period_category);
    setPeriod(ani?.period);
    setName(ani?.name);
    setAge(ani?.age);
    setMed(ani?.med_info);
    setAbout(ani?.about);
          ...
          >
            <Grid container spacing={2}>
              ...
              <Grid item xs={12}>
                ...
                >
                  <MenuItem value={ani.color}>-</MenuItem>
